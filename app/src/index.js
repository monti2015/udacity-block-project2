...
const App = {
    ...
    setStatus: function (message, _id) {
        const status = document.getElementById(_id);
    ...
    burnStar: async function () {
        const { burnStar } = this.meta.methods;
        const name = document.getElementById("starName").value;
        let id = document.getElementById("starId").value;
        id = parseInt(id);
        await burnStar(id).send({ from: this.account });
        App.setStatus(`${name} ${id} is deleted by ` + this.account + ".", "status");
    },

    lookUpStar: async function () {
        const { lookUptokenIdToStarInfo } = this.meta.methods;
        const id = document.getElementById("findStar").value;
        const name = await lookUptokenIdToStarInfo(id).call();
        App.setStatus(`${name}`, "status");
    }
